...
export function CouponCard({ recommendation, couponStatus, onRestart }: CouponCardProps) {
  return <section aria-label="已領取示範優惠券" style={{ marginTop: 20 }}>
    ...
    <article style={{ background: '#fffaf3', border: '2px dashed #b77d44', borderRadius: 16, marginTop: 14, padding: 20, textAlign: 'center' }}>
      ...
      <div aria-label="示範券碼" style={{ background: '#422716', borderRadius: 8, color: '#fff', fontFamily: 'monospace', letterSpacing: 2, padding: 13 }}>{couponStatus === 'redeemed' ? '已核銷' : 'WCH-2026-88'}</div>
      <p style={{ color: '#695747', fontSize: 13, lineHeight: 1.5 }}>此券為競賽展示資料，不可兌換、不可折現；可於店家端用同券碼展示一次性核銷。</p>
    </article>
